...
?>
    ...
    <div class="ccm-dashboard-header-buttons">
        <div class="btn-group">
            ...
            <?php
            if ($allowSuspend) { ?>
                <button type="button" class="btn btn-secondary text-danger" data-dialog="suspend-site"><?= t(
                        'Suspend'
                    ) ?></button>
            <?php
            } ?>
            <?php
            if ($allowReinstate) { ?>
                <button type="button" class="btn btn-secondary text-success" data-dialog="reinstate-site"><?= t(
                        'Reinstate'
                    ) ?></button>
            <?php
            } ?>
    ...
    <div style="display: none">
        <div id="ccm-dialog-delete-skyline-site" data-dialog-wrapper="delete-site" class="ccm-ui">
            <form method="post" action="<?= $view->action('delete', $hostingSite->getID()) ?>">
                ...
                <p><?= t(
                        'Are you sure you want to delete this hosting site record? <b>This will remove the hosting site from its server</b>.'
                    ) ?></p>
                <div class="dialog-buttons">
                    <button class="btn btn-secondary" onclick="jQuery.fn.dialog.closeTop()"><?= t('Cancel') ?></button>
                    <button class="btn btn-danger ms-auto"
                            onclick="$('#ccm-dialog-delete-skyline-site form').submit()"><?= t(
                            'Delete Site'
                        ) ?></button>
                </div>
            </form>
    ...
    <div style="display: none">
        <div id="ccm-dialog-reinstate-skyline-site" data-dialog-wrapper="reinstate-site" class="ccm-ui">
            <form method="post" action="<?= $view->action('reinstate', $hostingSite->getID()) ?>">
                ...
                <input type="hidden" name="id" value="<?= $hostingSite->getID() ?>">
                <p><?= t(
                        'Are you sure you want to reinstate this hosting site record? The site will be brought back online.'
                    ) ?></p>
                <div class="dialog-buttons">
                    <button class="btn btn-secondary" onclick="jQuery.fn.dialog.closeTop()"><?= t('Cancel') ?></button>
                    <button class="btn btn-success ms-auto"
                            onclick="$('#ccm-dialog-reinstate-skyline-site form').submit()"><?= t(
                            'Reinstate Site'
                        ) ?></button>
                </div>
            </form>
    ...
    <div style="display: none">
        <div id="ccm-dialog-suspend-skyline-site" data-dialog-wrapper="suspend-site" class="ccm-ui">
            <form method="post" action="<?= $view->action('suspend', $hostingSite->getID()) ?>">
                ...
                <input type="hidden" name="id" value="<?= $hostingSite->getID() ?>">
                <p><?= t(
                        'Are you sure you want to suspend this hosting site record? The site will be inaccessible, but the data and files will remain. The site may be reinstated at a future point, but suspended sites are automatically removed after %s days',
                        $_ENV['SKYLINE_DAYS_AFTER_SUSPENDING_TO_KEEP_SITE']
                    ) ?></p>
                <div class="dialog-buttons">
                    <button class="btn btn-secondary" onclick="jQuery.fn.dialog.closeTop()"><?= t('Cancel') ?></button>
                    <button class="btn btn-danger ms-auto"
                            onclick="$('#ccm-dialog-suspend-skyline-site form').submit()"><?= t(
                            'Suspend Site'
                        ) ?></button>
                </div>
            </form>
...
<?php
    ...
    <fieldset class="mb-5">
        ...
        <div class="row mb-3 d-flex align-items-center">
            ...
            <div class="col-md-9">
                <input type="text" class="form-control bg-white" readonly value="<?= $hostingSite->getHandle() ?>"
                       onclick="this.select()">
            </div>
        </div>
        <div class="row mb-3 d-flex align-items-center">
            ...
            <div class="col-md-9">
                <div class="input-group">
                    <input type="text" class="form-control bg-white" readonly
                           value="<?= $hostingSite->getPublicUrl() ?>" onclick="this.select()">
                    <a target="_blank" href="<?= $hostingSite->getPublicUrl() ?>" class="px-2 btn btn-secondary"><i
        ...
        <div class="row mb-3 d-flex align-items-center">
            ...
            <div class="col-md-9">
                <div class="input-group">
                    <input type="text" class="form-control bg-white" readonly
                           value="<?= $hostingSite->getControlPanelUrl() ?>" onclick="this.select()">
                    <a target="_blank" href="<?= $hostingSite->getControlPanelUrl() ?>"
                       class="px-2 btn btn-secondary"><i class="fa fa-external-link-alt"></i></a>
                </div>
            </div>
        </div>
    </fieldset>
    ...
    <fieldset class="mt-5 mb-5">
        ...
        <div class="row mb-3">
            ...
            <div class="col-md-9"><?= $hostingSite->getSubscriptionStatus() ?> (<a
                        href="https://stripe.com/docs/api/subscriptions/object#subscription_object-status"
                        target="_blank">Stripe Docs <i class="fa fa-external-link-alt"></i></a>)
            </div>
        </div>
        <div class="row mb-3">
            <div class="col-md-3 text-end"><b><?= t('Subscription ID') ?></b></div>
            <div class="col-md-9"><a target="_blank"
                                     href="https://dashboard.stripe.com/subscriptions/<?= $hostingSite->getSubscriptionId(
                                     ) ?>"><?= $hostingSite->getSubscriptionId() ?></a></div>
        </div>
    ...
    <fieldset class="mt-5 mb-5">
        ...
        <?php
        $backups = $hostingSite->getBackups()->toArray();
        if (count($backups)) { ?>

            <table class="table" id="skyline-backups">
                <thead>
                <tr>
                ...
                <tr v-for="backup in backups" :key="backup.id">
                    <td>{{backup.dateCreatedString}}</td>
                    <template v-if="backup.isLoading">
                        <td><span class="text-muted"><?= t('Pending...') ?></span></td>
                        <td colspan="2" class="align-middle">
                            <div class="d-flex"><i class="ms-auto fa fa-spinner fa-spin"></i></div>
                        </td>
                    </template>
                    <template v-else>
                        <td>{{backup.sizeString}}</td>
                        <td><a :href="backup.dashboardDownloadUrl"
                               class="ccm-hover-icon"><i class="fa fa-download"></i></a></td>
                        <td><a href="javascript:void(0)" data-dialog-title="<?= t('Delete Backup') ?>"
                               :data-dialog="'delete-backup-' + backup.id" class="ccm-hover-icon"><i
                ...
                <div style="display: none">
                    <div data-dialog-wrapper="delete-backup-<?= $backup->getID() ?>" class="ccm-ui">
                        <form method="post" action="<?= $view->action('delete_backup', $backup->getID()) ?>">
                            ...
                            <div class="dialog-buttons">
                                <button class="btn btn-secondary" onclick="jQuery.fn.dialog.closeTop()"><?= t(
                                        'Cancel'
                                    ) ?></button>
                                <button class="btn btn-danger ms-auto"
                                        onclick="$('div[data-dialog-wrapper=delete-backup-<?= $backup->getID(
                                        ) ?>] form').submit()"><?= t('Delete Backup') ?></button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            Concrete.Vue.activateContext('backend', function (Vue, config) {
                new Vue({
                    ...
                    mounted() {
                        ...
                        ConcreteEvent.subscribe('ConcreteServerEventSkylineBackupUpdated', function(e, data) {
                            ConcreteAlert.notify({
                                message: "Backup Created Successfully."
                            })

                            my.backups.forEach(function(thisBackup, index) {
                                if (thisBackup.id === data.backup.id) {
                                    thisBackup.isLoading = false
                                    thisBackup.size = data.backup.size
                                    thisBackup.dashboardDownloadUrl = data.backup.dashboardDownloadUrl
                                    thisBackup.sizeString = data.backup.sizeString
                                }
                            })
    ...
    <fieldset class="mt-5">
        ...
        <div class="card">
            <div class="card-body" vue-skyline>
                <skyline-installation-progress
                        :site='<?= h(json_encode($hostingSite)) ?>'></skyline-installation-progress>
            </div>
        </div>
    </fieldset>
<?php
